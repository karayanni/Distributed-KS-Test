...
# todo: fix the static method below.
class DigestCalculator:
    # Static variable for the reference digest
    reference_digest = TDigest()

    @staticmethod
    def initialize_reference_digest(ref_data):
        # Initialize reference digest only if it's empty (assuming 'is_empty' is a method to check this, or we could check for size == 0)
        DigestCalculator.reference_digest.batch_update(ref_data)

    @staticmethod
    def get_digest_ks(t_data):
        test_digest = TDigest()
        test_digest.batch_update(t_data)

        max_cdf = 0
        for item in DigestCalculator.reference_digest.centroids_to_list():  # centroids should cover the reference data space
            curr = abs(DigestCalculator.reference_digest.cdf(item['m']) - test_digest.cdf(item['m']))
            if curr > max_cdf:
                max_cdf = curr
...
def plot_errors_lines(ground_t_l, bins_l, digest_l, figure_name="figure"):
    ...
    plt.ylabel('Error compared to exact statistics')
...
def plot_exact_KS(ground_t_l, figure_name="figure"):
    k = len(ground_t_l)
    # Plot ground truth
    plt.plot(range(k), ground_t_l, label='Exact KS statistic', linestyle=(0, (1,10)), marker='o')
